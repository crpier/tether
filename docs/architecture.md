...
**Data layer** — **SQLite owns canonical Evidence, typed vertical state, Dream runs/cursors, suppressions, and complete Memory mutation/history records**, accessed through snekql. Current Memory is the recorded, Dreaming-authored Markdown workspace; there is no `Memory` row, trust state, facet table, or Todo→Memory link. The Ledger Vertical stores exact user-approved flat schema revisions and immutable evidence-linked entries; corrections append full replacements with explicit supersession rather than updating history. Source integrations retain scoped Evidence: Messages, retained historical email snapshots, Readwise highlights, reading progress, Health summaries, and other typed records. Ordinary Gmail ingestion records are triage and audit state, not Evidence; new durable Gmail-derived conclusions enter through settled final assistant Messages.

**Health observations.** Health Connect keeps append-only raw Telemetry and current typed projections in `telemetry.sqlite3`. It also retains Health Connect's source-priority-deduplicated activity projection because only the phone knows that priority order. Its insight module turns bounded current records into deterministic sleep episodes, local sleep days, stage composition, efficiency, sleep-aligned heart rate, comparable seven-day windows, and personal baselines. Chat receives those compact observations, completeness counts, and exact Evidence URIs instead of joining raw sensor records. Dreaming receives the same distinction between computed measurements and interpretation, requires at least three comparable episodes for a pattern, and cannot make clinical claims. Tether does not invent an opaque health score.

**Search** — current Memory Search scans validated Topic files through the workspace service, ranks direct lexical title/body matches, and always reconciles bytes against recorded Dreaming state first. This small-corpus path guarantees that a fresh Dream mutation affects the next action without waiting for an index. Ledger queries read structured entries through their own bounded filters and do not broaden Memory Search. Bucket-item and YouTube semantic Search retain their independent rebuildable LanceDB projections and local FastEmbed embeddings; they do not confer Memory authority.
...
**Background runtime** owns application-lifetime process work in one AnyIO task group. Callers register named finite passes or domain workers with an explicit retry or host-failure policy. The runtime handles deferred boot, readiness, cadence, capped retry backoff, health state, and ordered shutdown before SQLite closes. Durable Conversation turns, Scheduled occurrences, Dream runs, and Health Dream runs keep their own domain state and recovery rules; the runtime only drives them.
...
**Memory workspace** — canonical, recursively organized Markdown under `/data/kb/memory` (ADR 0021). Dreaming is its sole writer (ADR 0026). Topic files require YAML frontmatter and exact Evidence citations; meaningful paths are current identities. The web app resolves cited Messages, retained historical email snapshots, and exact historical Health Connect episode versions through one Evidence inspector. It collapses each Topic's complete provenance set until requested. SQLite records complete versions/tombstones and authorized mutations. Reads and startup reconciliation restore unauthorized edits/deletions, remove unknown valid files, and preserve exact recoverable pre-acknowledgement Dream mutations. An explicitly confirmed operator rebuild can tombstone Message-only Topics, reset Conversation assimilation, and replay retained Conversations under the current Dreaming policy. Topics with non-Conversation Evidence and all Dream history remain intact. Obsidian and Neovim are read-only inspection clients; corrections enter as Messages and queue Dreaming.
