...
Tether's hybrid Search (lexical + semantic, merged via RRF) is served by an **embedded LanceDB dataset** under `.tether/index/`, rather than the previously-planned in-SQLite FTS5 + `sqlite-vec` virtual tables. SQLite remains the single source of truth (ADR 0003): it holds Memory rows *and* the canonical embedding vectors (BLOB); LanceDB is a **derived projection** — gitignored and rebuildable from SQLite on restore (no re-embed) — strictly parallel to the markdown KB. Operational note: the current coarse `/data/kb` backup incidentally includes the index directory, but restoration never depends on that copy. This refines, but does not reverse, ADR 0003: the one clause that placed "the retrieval index" inside SQLite no longer holds; everything else about 0003 stands.
...
**Refinement (ADR 0021):** LanceDB remains a disposable candidate generator, but current canonical Memory files replace SQLite Memory rows/vectors as its rebuild source. Search must hydrate and revalidate candidates against current file path/content hashes.
