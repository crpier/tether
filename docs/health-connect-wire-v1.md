# Health Connect sync wire contract v1
...
must equal `1`; unknown versions and fields are rejected by the host contract.
...
`apps/host/tests/fixtures/health_connect/v1/representative-batch.json`.
...
- `POST /sync-state/baselines/complete` carries the expected token/generation
  ...
  `end_time` and complete `seen_record_ids`. Missing current IDs wholly inside
  those bounds become tombstones; records outside remain current. Completion
