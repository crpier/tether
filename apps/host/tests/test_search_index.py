"""Projection tests for the SearchIndex over HybridLanceTable.

`SearchIndex` is a thin Memory-shaped projection of `HybridLanceTable`
(`SearchDocument` in, `SearchCandidate` out; no extra payload columns). The
generic retrieval, lifecycle, and salvage behaviors are proven once in
`test_hybrid_lance_table.py`; these tests pin only what the projection adds —
its candidate mapping and its no-argument self-healing `optimize()` (the
reconcile loop calls it without a run-scoped logger).
"""
...
@test()
async def a_hit_maps_to_a_memory_candidate() -> None:
    """A search hit surfaces as a `SearchCandidate`: the Memory id + RRF score."""
    async with TemporaryDirectory() as tmp:
        ...
        assert_eq(candidates[0].id, memory_id)
...
async def optimize_self_heals_the_lance_corruption() -> None:
    ...
    into `HybridLanceTable` the Memory index had a bare optimize and the same
    corruption would have wedged the loop forever."""
    async with TemporaryDirectory() as tmp:
        index = await SearchIndex.open(index_dir=Path(tmp) / "index", vector_dim=_DIM)
        kept = SearchDocument(
            id=uuid4(), content="dentist appointment", vector=[1.0, 0.0, 0.0, 0.0]
        )
