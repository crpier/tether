...
class _GmailAuthRuntime(Protocol):
...
def _runtime(request: Request) -> _GmailAuthRuntime:
    """Read the installed application runtime off the request."""
    return cast("_GmailAuthRuntime", request.app.state.runtime)
...
async def gmail_auth_status(request: Request) -> GmailAuthStatus:
    ...
    auth_service = _runtime(request).gmail_auth_service
