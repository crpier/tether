    ...
    private fun render(platform: HealthConnectStatus, sync: HealthConnectSyncStatus) {
        ...
        binding.healthPermissions.setText(
            when {
                ...
                !available.permissions.canReadAllRecords -> R.string.health_connect_permissions_missing
        ...
        binding.healthSyncButton.isEnabled = available?.permissions?.canReadAllRecords == true && !sync.running
        if (available?.permissions?.canReadAllRecords == true) {
            scheduler.ensurePeriodicSync()
        }
    }
